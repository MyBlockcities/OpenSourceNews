...
const DailyFeedViewer: React.FC = () => {
    ...
    const loadDailyReports = async () => {
        try {
            setLoading(true);
            // Only look for reports we know exist (generated by the pipeline)
            const knownDates = ['2025-10-05', '2025-10-04']; // Add more as they're generated
            
            const reports: { date: string; data: DailyReport }[] = [];
            
            // Fetch reports (publicDir is set to 'outputs' in vite.config.ts)
            // So /daily/*.json maps to outputs/daily/*.json
            for (const date of knownDates) {
                try {
                    const response = await fetch(`/daily/${date}.json`);
                    if (response.ok) {
                        const data = await response.json();
                        reports.push({ date, data });
                        console.log(`✓ Loaded report for ${date}`);
                    }
                } catch (e) {
                    console.log(`Report not found for ${date}`);
                }
            }
